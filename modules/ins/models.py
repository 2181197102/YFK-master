...
class ins1_record_data(db.Model):
    ...
    # 基础健康信息
    smoking_history = db.Column(db.String(50), nullable=True, comment='吸烟史')
    drinking_history = db.Column(db.String(50), nullable=True, comment='饮酒史')
    hypertension_history = db.Column(db.String(50), nullable=True, comment='高血压史')
    blood_type = db.Column(db.String(50), nullable=True, comment='血型')
    heart_rate = db.Column(db.Numeric(10, 2), nullable=True, comment='心率')
    triglyceride = db.Column(db.Numeric(10, 2), nullable=True, comment='甘油三酯')
    total_cholesterol = db.Column(db.Numeric(10, 2), nullable=True, comment='总胆固醇')
    hdl_cholesterol = db.Column(db.Numeric(10, 2), nullable=True, comment='高密度脂蛋白胆固醇')
    ldl_cholesterol = db.Column(db.Numeric(10, 2), nullable=True, comment='低密度脂蛋白胆固醇')
    left_ventricular_ejection_fraction = db.Column(db.Numeric(10, 2), nullable=True, comment='左心室射血分数')
    serum_creatinine = db.Column(db.Numeric(10, 2), nullable=True, comment='血清肌酐')
    troponin = db.Column(db.Numeric(10, 2), nullable=True, comment='肌钙蛋白')

    # 诊断与治疗相关
    diagnosis_name_code = db.Column(db.String(50), nullable=True, comment='诊断名称及代码')
    planned_operation_code = db.Column(db.String(50), nullable=True, comment='拟实施手术及操作编码')
    transfusion_method = db.Column(db.String(50), nullable=True, comment='输血方式')
    drug_name = db.Column(db.String(50), nullable=True, comment='药物名称')
    medication_guidance = db.Column(db.String(50), nullable=True, comment='用药指导')
    lifestyle_guidance = db.Column(db.String(50), nullable=True, comment='生活方式指导')
    cost = db.Column(db.Numeric(10, 2), nullable=True, comment='费用')

    # 吸烟相关信息
    smoking_start_age = db.Column(db.Numeric(10, 2), nullable=True, comment='开始吸烟年龄(岁)')
    smoking_quit_age = db.Column(db.Numeric(10, 2), nullable=True, comment='戒烟年龄(岁)')
    occupational_dust_exposure = db.Column(db.String(50), nullable=True, comment='职业性烟尘接触史')
    environmental_risk_factors = db.Column(db.String(50), nullable=True, comment='环境危险因素史')

    # 肺功能检查
    fev1 = db.Column(db.Numeric(10, 2), nullable=True, comment='肺功能检查-第一秒用力呼气容积(FEV1)')
    fev1_fvc_ratio = db.Column(db.Numeric(10, 2), nullable=True, comment='肺功能检查-第一秒用力呼气容积与用力肺活量百分比(FEV1/FVC)')
    tlc = db.Column(db.Numeric(10, 2), nullable=True, comment='肺功能检查-总肺活量(TLC)')
    frc = db.Column(db.Numeric(10, 2), nullable=True, comment='肺功能检查-功能参气量(FRC)')
    dlco = db.Column(db.Numeric(10, 2), nullable=True, comment='肺功能检查-一氧化碳弥散量(DLCO)')
    dlco_va_ratio = db.Column(db.Numeric(10, 2), nullable=True, comment='肺功能检查-一氧化碳弥散量(DLCO)与肺泡通气量(VA)比值(DLCO/VA)')
    pef = db.Column(db.Numeric(10, 2), nullable=True, comment='肺功能检查-峰流速(PEF)')
    cat_score = db.Column(db.Numeric(10, 2), nullable=True, comment='慢阻肺评估测试(CAT)')
    mmrc_score = db.Column(db.Numeric(10, 2), nullable=True, comment='改良版英国医学研究委员会呼吸困难问卷(mMRC)评分')
    spo2 = db.Column(db.Numeric(10, 2), nullable=True, comment='脉搏血氧饱和度（SpO2)')

    # 家族与用药史
    family_history = db.Column(db.String(50), nullable=True, comment='家族史')
    medication_history = db.Column(db.String(50), nullable=True, comment='用药史')
    past_history = db.Column(db.String(50), nullable=True, comment='既往史')

    # 生活方式
    daily_staple_food = db.Column(db.Numeric(10, 2), nullable=True, comment='日主食量(g)')
    exercise_method = db.Column(db.Numeric(10, 2), nullable=True, comment='运动方式')
    exercise_duration = db.Column(db.Numeric(10, 2), nullable=True, comment='运动时长')

    # 血糖相关
    fasting_blood_glucose = db.Column(db.Numeric(10, 2), nullable=True, comment='空腹血糖值(mmol/L)')
    postprandial_blood_glucose = db.Column(db.Numeric(10, 2), nullable=True, comment='餐后两小时血糖值(mmol/L)')
    hypoglycemia_code = db.Column(db.Numeric(10, 2), nullable=True, comment='低血糖反应代码')
    hba1c = db.Column(db.Numeric(10, 2), nullable=True, comment='糖化血红蛋白值(%)')
    insulin = db.Column(db.Numeric(10, 2), nullable=True, comment='胰岛素')
    c_peptide = db.Column(db.Numeric(10, 2), nullable=True, comment='C肽')

    # 检查项目
    abdominal_ultrasound = db.Column(db.String(50), nullable=True, comment='腹部彩超')
    fundus_examination = db.Column(db.String(50), nullable=True, comment='眼底检查')
    lower_limb_vascular_ultrasound = db.Column(db.String(50), nullable=True, comment='下肢血管彩超')
    urine_microalbumin_creatinine_ratio = db.Column(db.Numeric(10, 2), nullable=True, comment='尿微量白蛋白/肌酐比值')
    arterial_stiffness = db.Column(db.Numeric(10, 2), nullable=True, comment='动脉僵硬度')

    # 血压相关
    bmi = db.Column(db.Numeric(10, 2), nullable=True, comment='BMI指数')
    waist_circumference = db.Column(db.Numeric(10, 2), nullable=True, comment='腰围')
    sitting_standing_blood_pressure = db.Column(db.Numeric(10, 2), nullable=True, comment='坐位、立位血压')
    ambulatory_blood_pressure_24h = db.Column(db.String(50), nullable=True, comment='24h动态血压')
    blood_uric_acid = db.Column(db.Numeric(10, 2), nullable=True, comment='血尿酸')
